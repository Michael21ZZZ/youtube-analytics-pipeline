...
def get_trends(keyword_path, keyword_ref, num_result):
    keyword_file = pd.read_csv(keyword_path) # Modified for different disease, Input the keyword list
    ...
    for i in range(num_iteration - 2):    
        normalised = normalised.drop(columns='sleep apnea causes'+"."+str(i+1)+"")
    ...
    avg_popularity.to_csv('./temp/top_100_keywords.csv')

if __name__ == '__main__':
    keyword_path = './input/input_list.csv' # Modified for different disease, Input the keyword list
    keyword_ref = 'sleep apnea causes' # use the first keyword for comparison in each iteration. A
    get_trends(keyword_path, keyword_ref, 100)